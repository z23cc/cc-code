...
ROUTE_TABLE = [
    ...
    (["dashboard", "overview", "status", "看板", "总览", "项目状态"],
     "dashboard", None, "Visual project overview"),
    (["verify", "lint", "check", "test run", "验证", "检查", "跑测试"],
     "verify", None, "Auto-detect language + lint + test"),
    (["search code", "find code", "grep", "搜代码", "找代码"],
     "search", None, "Semantic code search (Morph + grep)"),
    (["doctor", "diagnose", "check setup", "诊断", "检查环境"],
     "doctor", None, "10+ health checks"),
    (["graph", "dependency", "critical path", "dep graph", "依赖", "依赖图", "关键路径"],
     "graph", None, "Dependency visualization"),
    (["standup", "daily", "what happened", "日报", "昨天做了什么"],
     "standup", None, "Daily standup summary"),
    (["forecast", "eta", "when done", "预测", "什么时候完成"],
     "forecast", None, "ETA from velocity"),
    (["workflow", "pipeline", "chain", "工作流", "流水线"],
     "workflow list", None, "Multi-step workflow pipelines"),
    (["worktree", "parallel workspace", "isolated work", "工作树", "并行工作"],
     "/cc-worktree", None, "Git worktree management"),
    (["session", "save context", "resume", "保存会话", "恢复会话"],
     "session save", None, "Persist and restore session state"),
    (["rp", "repoprompt", "context builder", "代码分析", "RP"],
     "/cc-rp", None, "RepoPrompt integration"),
    (["memory", "remember", "recall", "记忆", "记住", "回忆"],
     "memory search", None, "Supermemory knowledge store"),
]
